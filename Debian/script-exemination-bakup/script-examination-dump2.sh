...
#1
ssh root@$ipbkp2 "docker-compose -f /srv/postgres/docker-compose.yml up -d " && echo "Постгрес поднят"
#2
pg_dump -h $ipprodgw -p 5433 -U bkgwuser -d bkgwdb | ssh root@$ipbkp2 "cat > $dirdump/bkgwdb" && echo "ДАМП СНЯТ"
#3
ssh root@$ipbkp2 "PGPASSWORD=$pgpasswd psql -h localhost -p 5432 -U bkgwuser -d test < $dirdump/bkgwdb" >> $histlog && echo "ДАМП РАЗВЕРНУТ"
...
then 
    ...
    ssh root@$ipbkp2 "rm $dirdump/bkgwdb &&\
...
#6
    ...
    rm $histlog
#7
    list=$(ssh root@$ipbkp2 "PGPASSWORD=$pgpasswd psql -h localhost -p 5432 -U bkgwuser -d test -c 'SELECT created_at FROM odds;'") > /dev/null && echo "Запрос в БД"
...
#9
        ...
        list=$(ssh root@$ipbkp2 "PGPASSWORD=$pgpasswd psql -h localhost -p 5432 -U bkgwuser -d test -c 'SELECT created_at FROM odds;'") > /dev/null
        echo "$list" | grep "$fdate" | sort | tail -15 >> $errlog
        ssh root@$ipbkp2 "docker stop db-examination-dump-prod && \
        docker rm db-examination-dump-prod && \
...
#10
        ...
        ssh root@$ipbkp2  "gzip -c9 $dirdump/bkgwdb > $dirdump/bkgwdb_$buckpdate.gz" && echo "Заархивированно"
        ssh root@$ipbkp2 "docker stop db-examination-dump-prod && \
        docker rm db-examination-dump-prod && \
...
#11
then
    echo "$buckpdate. Сервер bkp-2. Бэкап bkgwdb_$buckpdate проверен и сохранен." >> $log
fi
#12 
ssh root@$ipbkp2 "find $dirdump -mtime +2 -exec rm {} +"
